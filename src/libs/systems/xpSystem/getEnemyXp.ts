...
const getEnemyXp = (
    ...
    difficulty: 'low' | 'moderate' | 'high' = 'low',
): number[] => {
    let xpBudget = 0
    ...
    for (let i = ENEMY_XP.length - 1; i >= 0; i--) {
        if (ENEMY_XP[i] * enemyQuantity <= xpBudget) {
            return Array(enemyQuantity).fill(ENEMY_XP[i])
        }
    }

    throw new Error(
        `No element in ENEMY_XP can form a group of ${enemyQuantity} elements with sum <= ${xpBudget}`
    )
}
